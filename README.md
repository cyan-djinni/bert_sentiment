...
All reviews are tokenized with a maximum sequence length of 128.
...
A linear classification head is fine-tuned on top of the BERT encoder.
...
This step cleans the dataset and prepares it for training.
...
The slight decrease from validation to test performance indicates mild overfitting while maintaining strong generalization capability.
...
完整流程包括：数据预处理、模型微调、验证与测试评估、交互式预测。

最终模型在 Google Colab GPU 环境下完成全量训练。
...
Das finale Modell wurde mit dem vollständigen Datensatz unter GPU-Beschleunigung in Google Colab trainiert.
...
Die leichte Differenz zwischen Validierungs- und Testleistung deutet auf mildes Overfitting hin, bei insgesamt stabiler Generalisierungsfähigkeit.
...
La légère baisse entre validation et test indique un surapprentissage modéré tout en conservant une bonne capacité de généralisation.
```